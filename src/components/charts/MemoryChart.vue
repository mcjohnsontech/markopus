...
const { memoryChartData } = useMetrics()
  ...
  series: [
    {
      type: 'line',
      name: 'Memory Usage',
      data: memoryChartData.value.data,
      step: 'middle',
      ...
      itemStyle: { borderColor: '#a855f7', borderWidth: 0, color: '#a855f7' },
      symbolSize: 4,
      showSymbol: false,
    },
  ],
...
const fallbackState = computed(() => {
  if (store.connectionStatus !== 'connected' && memoryChartData.value.data.length === 0)
    return 'reconnecting'
  if (memoryChartData.value.data.length === 0) return 'no-data'
  return undefined
})
</script>

<template>
  <BaseChart :option="chartOption" :fallback-state="fallbackState" title="Memory Usage (%)" />
</template>
